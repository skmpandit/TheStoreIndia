...
const Productmanagement = () => {
  ...
  const { name, price, stock, photos, category, productDetails } = product;
        ...
        {isLoading ? ( <Skeleton length={20}/> ) : (
          <>
            <section> 
              ...
              <div>
                <img src={`${server}/${photos[currentIndex]}`} alt="Product" />
              </div>
              <div>
                <div>
                  {photos.map((photo, index) => (
                    <img key={index} src={`${server}/${photo}`} alt={`Product ${index}`} onClick={() => handleImageIndex(index)} className={`${currentIndex === index ? "active": ""}`} />
                  ))}
                </div>
              </div>
              <p>{name}</p>
